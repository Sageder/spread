...
def count_existing_markets(parquet_filename: str) -> int:
    """Count the number of existing markets in parquet file"""
...
def update_markets(parquet_filename: str = "data/markets.parquet", batch_size: int = 500):
    """
    Fetch markets ordered by creation date and save to parquet.
    Automatically resumes from the correct offset based on existing parquet records.
    ...
    current_offset = count_existing_markets(parquet_filename)
        ...
        try:
            ...
            for market in markets:
                try:
                    ...
                    batch_markets.append(market_dict)

                except (ValueError, KeyError, json.JSONDecodeError) as e:
                    print(f"Error processing market {market.get('id', 'unknown')}: {e}")
                    continue

            all_new_markets.extend(batch_markets)
            batch_count = len(batch_markets)
